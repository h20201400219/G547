/*Disk on File block device  with block device size 512 KB with sector size of 512 bytes having 2 primary partitions without any extended partitions */
...
{
	{
		...
		start_head: 0x00,
		start_sec: 0x2,
		start_cyl: 0x00,         //primary partition 1 start cylinder
		part_type: 0x83,
		end_head: 0x00,
		end_sec: 0x20,
		end_cyl: 0x09,
	...
	},
	{
		...
		start_head: 0x00,
		start_sec: 0x1,
		start_cyl: 0x14,          //primary partition 2 start cylinder
		part_type: 0x83,
		end_head: 0x00,
		end_sec: 0x20,
		end_cyl: 0x1F,
...
/** This is the registration and initialization section of the file block device driver **/
static int __init sbull_init(void)
{
	sbull_major = register_blkdev(sbull_major, "dof");         // registering our device
	if (sbull_major <= 0) {
	...
	spin_lock_init(&device.lock);	 // Get a request queue (here queue is created) 
	...
	device.gd->major = sbull_major;         // Setting the major number
	device.gd->first_minor = 0;             // Setting the first mior number
	device.gd->minors = SBULL_MINORS;
	device.gd->fops = &fops;                  // Initializing the device operations 
	device.gd->queue = dev->queue;        // Driver-specific own internal data 
	device.gd->private_data = dev;        // Driver-specific own internal data 
	sprintf(device.gd->disk_name,"dof");
	set_capacity(device.gd, nsectors*(hardsect_size/KERNEL_SECTOR_SIZE)); // Setting the capacity of the device in its gendisk structure
	add_disk(device.gd);	 // Adding the disk to the system    
	return 0;
}

/* * This is the unregistration and uninitialization section of the file block device driver */
...
MODULE_DESCRIPTION("DISK ON FILE BLOCK DRIVER");
